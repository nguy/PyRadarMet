...
def r_effective(dNdH=-39e-6):
 ...
 INPUT::
  dNdH            = Refraction [N x10^-6/km]
 OUTPUT::
  R1              = Effective radius [m]
 USAGE::
 ...
 NOTES::
  Effective radius of earth given a refraction.  If no refraction is given
   ...
   radar rays.
    """
# HISTORY::
#   3 Feb 2014 - Nick Guy NOAA/NSSL/WRDD, NRC
#---------------------------------------
...
def half_power_radius(r,bwhalf):
 ...
 INPUT::
  r            = Range [m]
  bwhalf       = Half-power beam width [degrees]
 OUTPUT::
  Rhalf        = Half-power radius [m]
 USAGE::
...
def ray_height(r,elev,H0,R1=R43):
 ...
 INPUT::
  r               = Range from radar to point of interest [m]
  elev            = Elevation angle of radar beam [deg]
  H0              = Height of radar antenna [m]
  R1              = Effective radius
 OUTPUT::
  H               = Radar beam height [m]
 USAGE::
...
def sample_vol_ideal(r,bwH,bwV,pLength):
 ...
 INPUT::
  ...
  bwH             = Horizontal beamwidth [deg]
  bwV             = Vertical beamwidth deg]
  pLength         = Pulse length [m]
 OUTPUT::
  SVol            = Sample Volume [m^3]
 USAGE::
...
def range_correct(r,h,E):
 ...
 INPUT::
  r               = Distance to sample volume from radar [m]
  h               = Height of the center of radar volume [m]
  E               = Elevation angle [deg]
 OUTPUT::
  rnew            = Adjusted range to sample volume [m]
 USAGE::
...
def beam_block_frac(Th,Bh,a):
 ...
 INPUT::
  Th              = Terrain height [m]
  Bh              = Beam height [m]
  a               = Half power beam radius [m]
 OUTPUT::
  PBB             = Partial beam blockage fraction [unitless]
 USAGE::
