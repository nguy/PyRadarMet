...
This program was ported from IDL code used at Colorado State University.
It is believed to be  originally written at CSU by Steven Nesbitt.
...
class BeamBlock(object):
    ...
    def __init__(self, dem_filename=None, verbose=False,
        ...
        dem_hdr_file : str
            Full path and filename of Digital Elevation Model header file.
...
#        self._grab_dem_file()
    ...
    def read_gt30_dem(self):
        ...
        '''
        if isinstance(self.dem_hdr_file, str) is not False:
            # wrap header in dict, because the different elements aren't
        ...
        for r in range(0, nrows):
            for c in range(0, ncols):
                ...
                if (elevation == 65535 or elevation < 0 or elevation > 20000):
                    # may not be needed depending on format, and the
                    # "magic number" value used for 'void' or missing data
                    elevation = 0.
        ...
        self.topo = topo[np.min(latInd):np.max(latInd),
                         np.min(lonInd):np.max(lonInd)]
        ...
        if self.ralt is None:
            ...
            radar_alt_map = scim.map_coordinates(
                self.topo, np.vstack((radar_lat_index, radar_lon_index)),
                prefilter=False)
    ...
    def calc_beam_blockage(self):
        ...
        self.h = ray_height(
            self.rng, self.E, self.ralt, reff=self.Reff)
        ...
        for jj, az in enumerate(self.phis):
            ...
            self.rng_lat[jj, :] = np.degrees(
                np.arcsin(np.sin(np.radians(self.rlat)) *
            ...
            self.terr[jj, :] = scim.map_coordinates(
                self.topo, np.vstack((self.inY, self.inX)), prefilter=False)

            # Calculate PBB along range
            self.PBB[jj, :] = beam_block_frac(self.terr[jj, :], self.h, self.a)
        ...
        for ii in range(self.nrng):
            if ii == 0:
                ...
                self.CBB[:, ii] = np.fmax([self.PBB[:, ii]],
                                          [self.CBB[:, ii-1]])
    ...
    def _check_dem_file(self):
        ...
        for dem_hdr_file in sorted(glob(os.path.join(self.demdir, '[e,w]*.hdr'))):
            ...
            dem_bbox = [[upper_left_x, lower_right_y],
        ...
        command = ['gdalbuildvrt', '-overwrite', '-te', str(minlon),
                   str(minlat), str(maxlon), str(maxlat),
                   os.path.join(self.demdir, 'interim.vrt')]
        ...
        if self.dem_file is None:
            ...
            self.dem_hdr_file = os.path.join(
                self.demdir, (dem_filename + ".hdr"))
    ...
    def plot_3panel(self, beam_blockage='complete',
        ...
        elev_min : float
            Minumum elevation to display on beam propagation and
            terrain height [meters]
        elev_max : float
            Maximum elevation to display on beam propagation and
            terrain height [meters]
        range_rings : float
        ...
        self.minlat = self.rlat - lat_plot_range
        self.maxlat = self.rlat + lat_plot_range
        self.minlon = self.rlon - lon_plot_range
        self.maxlon = self.rlon + lon_plot_range
        ...
        self.draw_terrain_height_map(
            fig, ax1, vmin=ht_shade_min, vmax=ht_shade_max,
    ...
    def draw_terrain_height_map(self, fig, ax, vmin=None, vmax=None,
        ...
        bm1 = Basemap(projection='cea', resolution='l', area_thresh=10000.,
                      ...
                      llcrnrlat=self.minlat, urcrnrlat=self.maxlat, ax=ax)
        ...
        Htmap = bm1.pcolormesh(xbm1, ybm1, self.topo/1000., vmin=topomin,
                               vmax=topomax, cmap=self.terr_cmap)
    ...
    def draw_bb_map(self, fig, ax, BB=None, range_rings=None,
        ...
        bm2.drawmeridians(np.arange(self.minlon, self.maxlon, lon_spacing),
                          labels=[1, 0, 0, 1])
        bm2.drawparallels(np.arange(self.minlat, self.maxlat, lat_spacing),
                          labels=[1, 0, 0, 1])
        bm2.drawcountries()
        bm2.drawcoastlines()
        bm2.drawrivers()
    ...
    def draw_beam_terrain_profile(self, fig, ax, ymin=None, ymax=None):
        ...
        tf = ax.fill_between(self.rng_gnd / 1000., 0.,
                             self.terr[self.Az_plot, :] / 1000., color='0.75')
    ...
    def save_map_data(self, filename=None):
        ...
        topoid = nc_fid.createVariable(
            'topography', np.float32, ('azimuth', 'range'))
        topoid.units = 'meters'
        topoid.long_name = 'Terrain Height'
        topoid[:] = self.terr[:]

        lonid = nc_fid.createVariable(
            'longitude', np.float32, ('azimuth', 'range'))
        lonid.units = 'degrees east'
        lonid.long_name = 'Longitude'
        lonid[:] = self.rng_lon[:]

        latid = nc_fid.createVariable(
            'latitude', np.float32, ('azimuth', 'range'))
