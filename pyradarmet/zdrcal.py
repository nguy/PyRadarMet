...
Implemented a more robust processing to reduce noise in data.
Thanks to Joseph Hardin regarding processing and posting a notebook online
...
def calculate_zdr_offset(radar, debug=False,
    ...
    ----------
    radar : Radar
        Radar object (from PyArt) to use for attenuation calculations.
        Must have copol_coeff, norm_coherent_power, proc_dp_phase_shift,
    ...
    sig_min : float
        Minimum standard deviation to consider valid and include
        in calculations.
    dbz_min : float
    ...
    rng = radar.range  # Range along ray
...
#    nsweeps = int(radar.nsweeps)
    ...
    ZDR, dBZ, Rhv, PhiDP, KDP = mask_gates(
        ZDR, dBZ, Rhv, PhiDP, KDP, rng['data']/1000., azimuth['data'], axAz,
    ...
    # Find individual range properties at a constant distance [height] from radar
    ...
    # Apply mask for data with range averaged Std Dev and azimuthal
    # averaged Std Deviations greater than threshold
    ZDR, dBZ, Rhv, PhiDP, KDP = mask_sigma(
        ZDR, dBZ, Rhv, PhiDP, KDP, DR_HtStd, DR_RayStd, axAz, minSig=sig_min)
    ...
    if (NGood > sample_min):
        ...
        zdr_bias = create_dict(
            DR_Avg, DR_Std, NGood, DR_RaySum, DR_RayAvg, DR_RayStd, NGood_Ray,
            DR_HtAvg, DR_HtStd, NGood_Ht, RH_HtAvg, dBZ_HtAvg, KDP_HtAvg,
            ZDR, DR_HtAvg_filt, DR_HtStd_filt, RH_HtAvg_filt, dBZ_HtAvg_filt,
            KDP_HtAvg_filt, PhiDP_HtAvg_filt, RadName, GenName, Rlat, Rlon,
            RAlt, time, rng, azimuth)
...
def create_dict(DR_Avg, DR_Std, NGood,
                ...
                RH_HtAvg_filt, dBZ_HtAvg_filt, KDP_HtAvg_filt,
                PhiDP_HtAvg_filt, RadName, GenName, Rlat, Rlon, RAlt,
                time, rng, azimuth):
...
def plot_3panel(X1, X2, X3, Y,
                ...
                X1lab='dBZ', X2lab='ZDR', X3lab=r'Kdp (dB km$^{-1}$)',
                ylab=None, label=True, labelTx=' ', labelpos=None):
    """
