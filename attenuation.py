...
def abs_coeff(D,lam,m):
 ...
 OUTPUT::
  Qa            = Absorption coefficient [unitless]
 USAGE::
  Qa = abs_coeff(D,lam,m)
 NOTES::
  An example from Battan (1973) is for water at 0C m=7.14-2.89j for a
   wavelength of 3.21 cm and for ice m=1.78-0.0024j for wavelength range from
   1-10 cm.
  ...
  Also see Doviak and Zrnic (1993), Fig. 3.3 caption.
    """
# HISTORY::
#   7 Feb 2014 - Nick Guy NOAA/NSSL/WRDD, NRC Postdoc
#---------------------------------------
    Km = (m**2 - 1) / (m**2 + 2)
...
def scat_coeff(D,lam,m):
 ...
 INPUT::
  ...
  m             = Complex refractive index [unitless]
 OUTPUT::
  Qs            = Scattering coefficient [unitless]
 USAGE::
...
def ext_coeff(D,lam,m):
 ...
 INPUT::
  ...
  m             = Complex refractive index [unitless]
 OUTPUT::
  Qe            = Scattering coefficient [unitless]
 USAGE::
